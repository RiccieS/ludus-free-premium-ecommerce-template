...
(function ($) {
    ...
    // ====== CLOUDFLARE TURNSTILE INJEKCE ======
    // Tento kód automaticky přidá Turnstile na každou stránku, kde je načten app.js

    window.addEventListener('load', function () {
        // 1. Vytvoříme neviditelný kontejner pro Turnstile a přidáme ho do body
        const tsContainer = document.createElement('div');
        tsContainer.id = 'turnstile-bg';
        tsContainer.style.cssText = 'position: fixed; bottom: 10px; right: 10px; z-index: 9999;';
        document.body.appendChild(tsContainer);

        // 2. Vytvoříme script tag, který stáhne Turnstile API z Cloudflare
        const tsScript = document.createElement('script');
        tsScript.src = "https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit";
        tsScript.async = true;
        tsScript.defer = true;

        // 3. Jakmile se skript z Cloudflare stáhne, inicializujeme ho
        tsScript.onload = function () {
            turnstile.render('#turnstile-bg', {
                sitekey: '0x4AAAAAACyH83ujKw2IvEtr', // NEZAPOMEŇ VYMĚNIT ZA SVŮJ KLÍČ!
                callback: function (token) {
                    console.log("✅ TURNSTILE SUCCESS: Skóre dobré, token vygenerován pro aktuální stránku.");
                },
                'error-callback': function () {
                    console.error("🚨 TURNSTILE BLOCKED: Bot detekován behaviorální analýzou!");
                }
            });
        };
